...
def build_constraint_matrix(n):
    """
        The constraint matrix encodes the constraint that each city (node) is connected to exactly two other cities in
        the output cycle.
    """
    m = int(n*(n-1)/2)
    C = np.zeros((m,m))
...
Problem = "9x9_P2"
in_file = "C:/Users/Admin/OneDrive/Dokumente/Fraunhofer/D-Wave/TSP by Jain/Own results/" + Problem + "/in_file.txt"
out_file = "C:/Users/Admin/OneDrive/Dokumente/Fraunhofer/D-Wave/TSP by Jain/Own results/" + Problem + "/out_file_qs.txt"
num_samples = 400

# the matrix of paiwise costs (cost to travel from node i to node j). this need not be a symmetric matrix but the
# diagonal entries are ignored and assumed to be zero (don't care)
